//! Helper functions for initializing and opening a RocksDB database.
...
/// Check if a database directory is empty.
///
/// A directory is considered empty if it doesn't exist or has no entries.
...
/// Creates a new database at the specified path if it doesn't exist.
/// Does NOT create tables. Use [`init_db`] for a complete setup.
...
/// Creates tables defined in [`Tables`] if necessary. Read/Write mode.
pub fn init_db<P: AsRef<Path>>(path: P, args: DatabaseArguments) -> eyre::Result<DatabaseEnv> {
    init_db_for::<P, Tables>(path, args)
}

/// Opens up an existing database or creates a new one at the specified path.
/// Creates tables defined in the given [`TableSet`] if necessary. Read/Write mode.
...
/// It doesn't create it or create tables if missing.
